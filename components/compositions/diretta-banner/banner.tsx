    ...
    <div className="w-full flex flex-col">
      ...
      <div className="text-foreground w-full p-3">
        <div className="w-full">
            <div>
                <Image height={56} width={56} src={fixture.league?.logo} alt={'league logo'} className="w-14 h-14" />
            </div>
          <div className="text-center text-xs text-muted-foreground mb-1">{fixture.league?.name} • {fixture.league?.round}</div>
          ...
          <div className="flex justify-between items-center py-4 px-6">
            <div className="flex-1 text-right pr-6">
              <div className="text-base font-bold">{fixture.teams?.home?.name}</div>
              <div className="text-xs text-muted-foreground">inserire citta</div>
            </div>
            ...
            <div className="flex items-center justify-center px-4">
              <div className="h-14 w-14 relative mr-4 flex-shrink-0">
                <div className="absolute p-3 inset-0 bg-white rounded-lg flex items-center justify-center text-white font-bold">
                  <Image height={56} width={56} src={fixture.teams?.home?.logo} alt={'home team logo'} className="w-14 h-14" />
                </div>
              </div>
              
              <div className="px-3 py-1 bg-accent rounded">
                <span className="text-xl font-bold">{fixture.score?.fulltime?.home ?? '-'}:{fixture.score?.fulltime?.away ?? '-'}</span>
              </div>
              ...
              <div className="h-14 w-14 relative ml-4 flex-shrink-0">
                <div className="absolute inset-0 bg-white rounded-lg flex items-center justify-center text-white font-bold">
                  <Image height={56} width={56} src={fixture.teams?.away?.logo} alt={'away team logo'} className="w-14 h-14" />
                </div>
              </div>
            </div>
            
            <div className="flex-1 text-left pl-6">
              <div className="text-base font-bold">{fixture.teams?.away?.name}</div>
              <div className="text-xs text-muted-foreground">inserire citta</div>
            </div>
          </div>
          
          {/* Data e ora - più compatta */}
          <div className="text-center mt-2 text-xs text-muted-foreground">{fixture.fixture?.date.split('T')[0]}</div>
        </div>
      </div>
    </div>
  );
